...
- **Latest release tag `v0.26.0`; `HEAD` is past it and untagged** — `git
  describe` is the live answer, so no commit count is written down here (one
  would be stale the moment this file is committed). Seven tags on 2026-08-25/26,
  `v0.20.0` → `v0.26.0`, each with a CHANGELOG entry naming its re-collaudo
