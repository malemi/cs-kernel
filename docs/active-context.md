...
- Clone matrix (verified 2026-08-25 from inside each clone —
  ...
  |---|---|---|---|
  | `mrcall-cs` | `v0.24.0` | OpenRouter → `z-ai/glm-5.3` | **PAUSED** since 2026-08-24 13:03 by its own tick: `cs chat --allow send_draft` ignored the draft id it was asked for, twice, and sent a different draft. The pause is still set and was NOT cleared by the re-pin. Three crons installed and live when un-paused — hourly signup loop and 2-hourly operator, both **sending**, plus the dormant July batch-2 lines |
  | `124-cs` | `v0.24.0` | Anthropic direct → `claude-sonnet-5` | Running, not paused. Cron installed, 2-hourly, draft-only |

  Both pin `v0.24.0` as of 2026-08-26, installed and verified from inside each
  clone (`cs --version` reports `0.24.0`), with `v0.24.0`'s FULL collaudo run
  on both — not waived; `mrcall-cs`'s own 8-file `ext/` suite is green too, and
  `cs unanswered --crm` was exercised on BOTH backends, which is the point of
  putting it behind the port: `starchat` renders `ACTIVE/essential`, `shopify`
  renders `0/0.0 EUR`, from the same code. The previous line, for `v0.23.0`,
  read the same way. The re-pin also repaired two stale claims per clone:
