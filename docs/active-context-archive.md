...
## 2026-09-14 — Snapshot before Vonage production adoption


- **Latest release tag: `v0.45.0`. Current HEAD status: untagged.** These
...
## 2026-09-10 — snapshot before workspace-setup reconciliation

The following snapshot is preserved verbatim. Deployment and clone observations
are historical reports, not checks performed during workspace-setup development.
...
The untagged development tree adds explicit `cs init --descriptor PATH`,
selected-profile mailbox handoff, truthful install/login outcomes, and
`cs setup [--json]`. Setup uses identity, preparation and memory read RPCs;
missing old-engine evidence stays unverified. Its email counts describe memory
processing, not reply quality. Agent executable presence does not verify agent
login. No new permission entries, versions, releases, or operational clone
upgrades accompany this work. [Delivery plan](execution-plans/2026-09-10-desktop-workspace-setup.md).
...
- Public-release onboarding on a fresh machine remains unverified. The development
  setup gate stamps an isolated workspace, installs the changed source into its
  own environment, and checks login/readiness against a controlled local engine;
  this does not prove public tag installation or live provider authentication.
